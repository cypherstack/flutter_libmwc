...
Future<void> main(List<String> arguments) async {
  final parser = ArgParser()
    ..addOption('output', defaultsTo: 'build/macos-prebuilts')
    ..addFlag('help', abbr: 'h', negatable: false);
  try {
    final args = parser.parse(arguments);
    if (args.flag('help')) {
      stdout.writeln(
        'Build native macOS prebuilts using Nix.\n${parser.usage}',
      );
      return;
    }
    ...
    if (!Platform.isMacOS) {
      throw StateError('The macOS producer requires a native macOS host');
    }
    ...
    await packageBuiltTarget(
      ...
      build: {
        ...
        'macos_recipe_sha256': await sha256File(
          File('reproducible/macos/package.nix'),
        ),
        'store_path': output,
      },
    );

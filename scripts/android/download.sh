...
download_and_verify "libmwc_wallet-android-arm64-v8a.so"
mkdir -p "$JNILIBS/arm64-v8a"
cp "$TMPDIR/libmwc_wallet-android-arm64-v8a.so" "$JNILIBS/arm64-v8a/libmwc_wallet.so"

download_and_verify "libmwc_wallet-android-armeabi-v7a.so"
mkdir -p "$JNILIBS/armeabi-v7a"
cp "$TMPDIR/libmwc_wallet-android-armeabi-v7a.so" "$JNILIBS/armeabi-v7a/libmwc_wallet.so"

download_and_verify "libmwc_wallet-android-x86_64.so"
mkdir -p "$JNILIBS/x86_64"
cp "$TMPDIR/libmwc_wallet-android-x86_64.so" "$JNILIBS/x86_64/libmwc_wallet.so"
